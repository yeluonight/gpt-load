export default {
  ...
  keys: {
    ...
    usageLimits: "用量限制",
    usageLimitsTooltip:
      "按模型限制每个 Key 的 RPM/TPM 和模型周期调用次数，也可单独设置每个 Key 的总周期调用次数。达到任一限制的 Key 会被跳过，优先尝试同组其他 Key。",
    modelLimit: "模型限制",
    modelLimitTooltip:
      "模型名支持精确匹配，填写 * 可作为默认限制。TPM 单位是 tokens/min，请填纯数字，例如 250000。每天固定时间按太平洋时间 PT 计算。",
    tpmPlaceholder: "TPM (tokens/min)",
    modelMaxRequests: "模型周期请求数",
    addModelLimit: "添加模型限制",
    modelRateLimitInvalid: "模型限制需要填写模型名，并至少设置 RPM、TPM 或模型周期请求数",
    keyRequestLimit: "Key 调用次数",
    keyRequestLimitTooltip:
      "限制每个 Key 在一个刷新窗口内最多可发起的上游请求总数，和模型周期请求数独立计算。每天固定时间按太平洋时间 PT 计算。0 或留空表示不限制。",
    maxRequests: "最大请求数",
    resetDaily: "每天固定时间 (PT)",
    resetByInterval: "按周期刷新",
    intervalMinutes: "周期分钟",
    keyRequestLimitInvalid: "Key 调用次数限制配置不正确，请检查刷新周期或时间点",
    proxyPool: "代理池",
    proxyPoolTooltip:
      "为此分组配置多个出站代理。系统会尽量让同一个 Key 固定使用同一个代理，并在多个 Key 并发时分散出口。配置代理池后优先于普通代理服务器地址。",
    proxyPoolInputTooltip: "每行一个代理 URL，也支持逗号分隔。可添加后逐条测试、禁用或删除。",
    ...
    proxyTestSuccess: "代理测试成功 ({duration})",
    proxyDisabled: "禁用",
    proxyPermanentDisabled: "长期禁用",
    proxyNotes: "备注",
    proxyUrlRequired: "请先填写代理地址",
